...
namespace B2BApp.Business.Abstract
{
    public class SubeStokService : ISubeStokService
    {
        ...
        public void addManySubeStok(List<SubeStok> subeStokList)
        {
            ...
            {
                _unitOfWork.SubeStok.Remove(_unitOfWork.SubeStok.GetFirstOrDefault(x=>x.Id == duplicateStok.Id).Data);
            }
        ...
        public void deleteSubeStok(Guid objectId)
        {
            try
            {
                ...
                _unitOfWork.SubeStok.Remove(_unitOfWork.SubeStok.GetFirstOrDefault(x=>x.Id == objectId).Data);
            }
            catch (Exception ex)
            {
        ...
        public Result<ICollection<SubeStokDto>> getAllWithDetailsByFilters(string? subeId, string? firmaId, string? kategoriId)
        {
            try
            {
                ...
                var sube = subeId == null ? _unitOfWork.Sube.GetAll().Data : _unitOfWork.Sube.GetAll(x => x.Id.ToString() == subeId).Data;
        ...
        public Result<ICollection<SubeStokDto>> getAllWithSubeAndUrun()
        {
            try
            {
                ...
                {
                    var urun = _unitOfWork.Urun.GetFirstOrDefault(x=>x.Id == subeStok.UrunId).Data;
                    var sube = _unitOfWork.Sube.GetFirstOrDefault(x=> x.Id == subeStok.SubeId).Data;
        ...
        public Result<SubeStok> getSubeStokById(Guid objectId)
        {
            try
            {
                ...
                return _unitOfWork.SubeStok.GetFirstOrDefault(x=>x.Id == objectId );
            }
            catch (Exception ex)
            {
        ...
        public Result<SubeStokDto> getWithSubeAndUrun(Guid objectId)
        {
            try
            {
                var subeStok = _unitOfWork.SubeStok.GetFirstOrDefault(x => x.Id == objectId).Data;
