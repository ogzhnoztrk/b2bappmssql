...
namespace B2BApp.Business.Abstract
{
    public class TedarikciService : ITedarikciService
    {
        ...
        public void deleteTedarikci(Guid objectId)
        {
            try
            {
                _unitOfWork.Tedarikci.Remove(_unitOfWork.Tedarikci.GetFirstOrDefault(x=>x.Id == objectId).Data);
        ...
        public Result<Tedarikci> getTedarikciById(Guid objectId)
        {
            try
            {
                ...
                return _unitOfWork.Tedarikci.GetFirstOrDefault(x=>x.Id == objectId);
            }
            catch (Exception ex)
            {
